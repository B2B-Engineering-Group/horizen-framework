...
/**
 * Весь фреймворк базируется на строгой типизации уровня API. Валидатор - основа этой зоны.
 * 
 * Предоставляет унифицированный интерфейс для разработки схем и валидации JSON и файлов. Обеспечивает 
 * набор базовых типов, позволяет добавлять кастомные. Генерирует универсальные схемы, 
 * которые можно экспортировать, а потом импортировать обратно.
 * 
 * TODO: добавить типы int float и т.д. Людей клинит от корневого number, требующего регулярку 
 * из-за необходимости валидировать разные представления чисел.
 * 
 * Использование смотреть в тестах.
 **/
